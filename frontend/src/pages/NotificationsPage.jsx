...
const RowSkeleton = ({ showCategory }) => (
...
const NotificationsPage = () => {
  ...
  // Prepare API params: "all" category is omitted
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody>
                  {Array.from({ length: 6 }).map((_, i) => (
                    <RowSkeleton key={i} showCategory={category === "all"} />
                  ))}
                </tbody>
              </table>
            ) : isError ? (
                      ...
                      >
                        ...
                        <td className="px-5 py-4">
                          ...
                          {(item.startsAt || item.endsAt) && (
                            <p className="text-xs text-gray-400 mt-1">
                              Valid: {formatDate(item.startsAt)} →{" "}
                        ...
                        {category === "all" && (
                          <td className="px-5 py-4 text-gray-500 text-xs whitespace-nowrap">
                            {item.category ? (
                              TABS.find((t) => t.key === item.category)
                                ?.label ?? item.category
                            ) : (
              ...
              {pagination && pagination.totalPages > 1 && (
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={page === 1}
                    className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-gray-600 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-white transition-colors"
                  >
