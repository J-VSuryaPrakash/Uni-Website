...
const Navbar = () => {
  ...
  return (
    ...
    >
              ...
              menuItems.map((item, index) =>
                item.children ? (
                  ...
                  >
                    ...
                    <div
                      className={`absolute pt-4 w-64 transform transition-all duration-200 ${index > menuItems.length - 2 ? 'right-0 origin-top-right' : 'left-0 origin-top-left'} ${openDropdown === item.label
      ...
      {/* MOBILE MENU */}
      <div
        className={`md:hidden fixed inset-x-0 bg-white/95 backdrop-blur-lg border-b border-gray-200 shadow-xl transition-all duration-300 ease-in-out origin-top ${mobileOpen ? "max-h-[80vh] opacity-100 py-4 overflow-y-auto" : "max-h-0 opacity-0 overflow-hidden py-0"
          }`}
      >
