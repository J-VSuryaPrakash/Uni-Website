...
export default function FacultyCard({ directorate }) {
    ...
    return (
        <div className="bg-white border border-gray-200 shadow-sm hover:shadow-md transition-shadow duration-200 flex gap-0 overflow-hidden" style={{ borderRadius: '4px' }}>
            ...
            {/* ── Left: Photo column ─────────────────────────────── */}
            <div className="flex-shrink-0 w-28 bg-gray-50 border-r border-gray-200 flex flex-col items-center justify-start pt-5 px-3 pb-4">
                {photoUrl ? (
                    <img
                        src={photoUrl}
                        alt={name}
                        className="w-20 h-24 object-cover object-top border border-gray-200"
                        style={{ borderRadius: '2px' }}
                    />
                ) : (
                    <div className="w-20 h-24 bg-gray-100 border border-gray-200 flex items-center justify-center" style={{ borderRadius: '2px' }}>
            ...
            <div className="flex-1 min-w-0 px-4 py-4 flex flex-col gap-1.5">
                ...
                {/* Name */}
                <h3 className="text-sm font-bold text-gray-900 leading-snug tracking-tight">
                    {name}
                </h3>
                ...
                {hasContact && (
                    <div className="flex flex-col gap-1 border-t border-gray-100 pt-1.5 mt-0.5">
                        {contact.email && (
                            ...
                            >
                                <svg className="h-3 w-3 flex-shrink-0 text-gray-400 group-hover:text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
