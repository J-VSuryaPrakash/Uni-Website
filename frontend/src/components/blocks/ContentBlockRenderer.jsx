    ...
    return (
        ...
        >
            ...
            >
                ...
                >
                    <X className="w-6 h-6 hover:cursor-pointer rounded-sm bg-red-600" />
                </button>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    {visibleImages.map((url, i) => (
                        <div
                            ...
                            onClick={() => setSelectedImage(url)} // ✅ click
                        >
                            <Image
                                src={url}
                                width={400}
                                height={300}
                                className="w-full h-56 object-cover rounded-lg shadow-sm border border-gray-400 group-hover:brightness-95 transition"
                            />
                        </div>
                    ))}
                </div>
...
const ListBlock = ({ content }) => {
  ...
  return (
    <div className="my-4">
      ...
      <Tag
        className={`${isOrdered ? "list-decimal" : "list-disc"} pl-6 space-y-1 text-gray-700`}
      >
        {items.map((item, i) => (
          <li key={i}>{item}</li>
        ))}
        ...
        <table className="w-full text-sm text-left">
          {headers.length > 0 && (
            <thead>
              <tr className="bg-blue-900 text-white">
                <th className="px-5 py-3 text-xs font-semibold uppercase tracking-wide border-r border-blue-800/40 last:border-r-0 whitespace-nowrap">
                  Sl.no
                </th>
                {headers.map((header, i) => (
                  <th
                    key={i}
                    className="px-5 py-3 text-xs font-semibold uppercase tracking-wide border-r border-blue-800/40 last:border-r-0 whitespace-nowrap"
                  >
                    {header}
                  </th>
                ))}
          ...
          <tbody className="divide-y divide-gray-100">
            {rows.map((row, i) => (
              <tr
                key={i}
                className={`hover:bg-blue-50/40 transition-colors ${i % 2 === 1 ? "bg-gray-50/60" : "bg-white"}`}
              >
                <td className="px-5 py-3 text-gray-400 font-medium text-xs whitespace-nowrap">
                  {i + 1}
                </td>
                {(Array.isArray(row) ? row : Object.values(row)).map(
                  (cell, j) => (
                    <td key={j} className="px-5 py-3 text-black text-sm">
                      {cell}
                    </td>
                  ),
                )}
...
const DirectorateBlock = ({ content }) => {
  const directorates = Array.isArray(content.directorates)
